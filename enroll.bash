...
if [ -f "$filename" ]; then
    ...
    # First, create a temporary file to update the course's details
    temp_file=$(mktemp)
    ...
    echo "$(date +"%Y-%m-%d %H:%M:%S") ENROLLMENT: $dept_code $course_num $(head -n 2 "$filename" | tail -n 1) changed by $change_amt" >> data/queries.log
